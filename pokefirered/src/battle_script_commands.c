...
#define DEFENDER_IS_PROTECTED (((gProtectStructs[gBattlerTarget].protected) || gProtectStructs[gBattlerTarget].kingsShielded || gProtectStructs[gBattlerTarget].spikyShielded) && (gBattleMoves[gCurrentMove].flags & FLAG_PROTECT_AFFECTED))
...
static void Cmd_setroomterrain(void);
static void Cmd_switchoutabilities(void);
static void Cmd_jumpifhasnohp(void);
static void Cmd_getsecretpowereffect(void);
static void Cmd_pickup(void);
static void Cmd_trymagnetrise(void);
...
static void Cmd_trycopycat(void);
...
void (* const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_setroomterrain,                          //0xE1
    Cmd_switchoutabilities,                      //0xE2
    Cmd_jumpifhasnohp,                           //0xE3
    Cmd_getsecretpowereffect,                    //0xE4
    Cmd_pickup,                                  //0xE5
    Cmd_trymagnetrise,                           //0xE6
    Cmd_tryworryseed,                            //0xE7
    ...
    Cmd_setroost,                                //0xF8
    Cmd_callnative,                              //0xF9
    Cmd_metalburstdamagecalculator,              //0xFA
    Cmd_setembargo,                              //0xFB
    Cmd_sethealblock,                            //0xFC
    Cmd_trycopycat,                              //0xFD
    Cmd_swapstatstages,                          //0xFE
    Cmd_trysetaquaring,                          //0xFF
};
...
static const u8 *const sMoveEffectBS_Ptrs[] =
{
    ...
    [MOVE_EFFECT_REMOVE_SLEEP]     = BattleScript_MoveEffectSleep,
...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_SUPER_POTION,    {   3,   6,  10,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_POKE_DOLL,       {   8,  10,   9,  30,   _,   _,   _,   _,   _,   _, } },
    { ITEM_BIG_MUSHROOM,    {   3,  10,   9,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_SUPER_REPEL,     {   9,  15,   9,   9,  30,   _,   _,   _,   _,   _, } },
    { ITEM_FULL_HEAL,       {   3,   3,   9,   8,   9,  10,   _,   _,   _,   _, } },
    { ITEM_REVIVE,          {   5,   5,  13,  11,  18,   9,  35,   _,   _,   _, } },
    { ITEM_HYPER_POTION,    {   _,   _,   _,   1,   1,   3,   3,   3,   _,   _, } },
    { ITEM_ETHER,           {   1,   1,   3,   4,   4,   _,   _,   _,   _,   _, } },
    { ITEM_MAX_REPEL,       {   _,   3,   8,   4,  14,   9,   8,   8,  30,   _, } },
    { ITEM_MOON_STONE,      {   _,   3,   3,   4,   4,   4,   4,  15,   9,  17, } },
    { ITEM_SUN_STONE,       {   _,   3,   3,   4,   4,   7,   7,  16,   9,  18, } },
    { ITEM_RARE_CANDY,      {   _,   1,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_NUGGET,          {   _,   _,  14,   9,  11,  10,   4,   5,   7,   9, } },
    { ITEM_MAX_POTION,      {   _,   _,   _,   _,   _,   _,   1,   1,   1,   1, } },
    { ITEM_MAX_ETHER,       {   _,   _,   1,   1,   4,   4,   4,   _,   _,   _, } },
    { ITEM_PP_UP,           {   _,   _,   1,   1,   1,   4,   4,   5,   4,   5, } },
    { ITEM_PP_MAX,          {   _,   _,   1,   1,   1,   1,   4,   5,   4,   5, } },
    { ITEM_X_ATTACK,        {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_LEFTOVERS,       {   _,   _,   1,   1,   5,   5,   8,  11,   9,   1, } },
    { ITEM_MENTAL_HERB,     {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_X_SPECIAL,       {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_WHITE_HERB,      {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_X_SPEED,         {   _,   _,   1,   4,   4,   4,   4,   5,   4,   5, } },
    { ITEM_MAX_REVIVE,      {   _,   _,   5,   4,   4,   4,   4,  14,   9,  23, } },
    { ITEM_ELIXIR,          {   _,   _,   4,   _,   1,   1,   4,   5,   4,   5, } },
    { ITEM_MAX_ELIXIR,      {   _,   _,   1,   _,   _,   _,   1,   1,   4,   5, } },
    { ITEM_FULL_RESTORE,    {   _,   _,   _,   _,   _,   _,   _,   1,   1,   1, } },
};
...
static void Cmd_attackcanceler(void)
{
     ...
     && !gProtectStructs[gBattlerAttacker].usesBouncedMove)
    {
        ...
        return;
    } else if (gBattleMons[gBattlerTarget].ability == ABILITY_MAGIC_BOUNCE 
           && (gBattleMoves[gCurrentMove].flags & FLAG_MAGIC_COAT_AFFECTED)
           && !gProtectStructs[gBattlerAttacker].usesBouncedMove)
    {
        gProtectStructs[gBattlerTarget].usesBouncedMove = TRUE;
...
static bool8 AccuracyCalcHelper(u16 move)
{
    ...
    if ((WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_RAIN) && (gBattleMoves[move].effect == EFFECT_THUNDER || gBattleMoves[move].effect == EFFECT_HURRICANE))
     ...
     || (gBattleMoves[move].effect == EFFECT_ALWAYS_HIT || gBattleMoves[move].effect == EFFECT_VITAL_THROW || gBattleMoves[move].effect == EFFECT_TRUMP_CARD || gBattleMoves[move].effect == EFFECT_EVASION_DOWN_3 || move == MOVE_PLAY_NICE || move == MOVE_CONFIDE))
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        {
            GET_MOVE_TYPE(gCurrentMove, type);
        }
        ...
        if (WEATHER_HAS_EFFECT && gBattleWeather & B_WEATHER_SUN && (gBattleMoves[move].effect == EFFECT_THUNDER || gBattleMoves[move].effect == EFFECT_HURRICANE))
            moveAcc = 50;
        ...
        if (gBattleMons[gBattlerTarget].ability == ABILITY_WONDER_SKIN && IS_MOVE_STATUS(move) && moveAcc > 50 && gBattleMons[gBattlerAttacker].ability != ABILITY_MOLD_BREAKER && gBattleMons[gBattlerAttacker].ability != ABILITY_TURBOBLAZE && gBattleMons[gBattlerAttacker].ability != ABILITY_TERAVOLT)
            calc = (calc * 50) / 100; // 1.5 wonder skin loss
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_VICTORY_STAR)
            calc = (calc * 110) / 100; // 1.1 victory star boost

        if (gBattleMons[gBattlerTarget].item == ITEM_ENIGMA_BERRY && gBattleMons[gBattlerTarget].ability != ABILITY_KLUTZ)
        {
            holdEffect = gEnigmaBerries[gBattlerTarget].holdEffect;
            param = gEnigmaBerries[gBattlerTarget].holdEffectParam;
        }
        else if (gBattleMons[gBattlerTarget].item != ITEM_ENIGMA_BERRY && gBattleMons[gBattlerTarget].ability != ABILITY_KLUTZ)
        {
...
static void Cmd_critcalc(void)
{
    ...
    if (gBattleMoves[gCurrentMove].effect == EFFECT_STORM_THROW)
    {
        critChance = 255; // Set critChance to a high number to ensure it always results in a critical hit
    }
    else
    {
        critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                    ...
                    + (gBattleMoves[gCurrentMove].effect == EFFECT_CROSS_POISON)
    ...
    if ((gBattleMons[gBattlerAttacker].ability == ABILITY_SNIPER && gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     ...
     && !(gBattleTypeFlags & BATTLE_TYPE_POKEDUDE))
        gCritMultiplier = 3;
    else if ((gBattleMons[gBattlerAttacker].ability != ABILITY_SNIPER && gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     && !(gStatuses3[gBattlerAttacker] & STATUS3_CANT_SCORE_A_CRIT)
     && !(gSideStatuses[gBattlerTarget] & SIDE_STATUS_LUCKY_CHANT)
     && !(gBattleTypeFlags & BATTLE_TYPE_OLD_MAN_TUTORIAL)
     && !(Random() % sCriticalHitChance[critChance])
...
static void Cmd_damagecalc(void)
{
    ...
    if (gStatuses3[gBattlerAttacker] & STATUS3_ME_FIRST)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
    if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN && gBattleMoves[gCurrentMove].type == TYPE_GRASS)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN && gBattleMoves[gCurrentMove].type == TYPE_DRAGON)
        gBattleMoveDamage = gBattleMoveDamage / 2;
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN && gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
    if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN && gBattleMoves[gCurrentMove].type == TYPE_PSYCHIC)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
...
void AI_CalcDmg(u8 attacker, u8 defender)
{
    ...
    if (gProtectStructs[attacker].helpingHand)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
    if (gStatuses3[attacker] & STATUS3_ME_FIRST)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
    if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN && gBattleMoves[gCurrentMove].type == TYPE_GRASS)
        gBattleMoveDamage = gBattleMoveDamage * 15 / 10;
    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN && gBattleMoves[gCurrentMove].type == TYPE_DRAGON)
...
{
    bool8 isFreezeDry = (gCurrentMove == MOVE_FREEZE_DRY);
    bool8 hasScrappy = (gBattleMons[gBattlerAttacker].ability == ABILITY_SCRAPPY);
    ...
    case TYPE_MUL_NO_EFFECT:
        if (hasScrappy && (gBattleMoves[gCurrentMove].type == TYPE_NORMAL || gBattleMoves[gCurrentMove].type == TYPE_FIGHTING)
        && (gBattleMons[gBattlerTarget].type1 == TYPE_GHOST || gBattleMons[gBattlerTarget].type2 == TYPE_GHOST))
        {
            // Normal and Fighting moves hit Ghost types if the attacker has Scrappy
    ...
    case TYPE_MUL_NOT_EFFECTIVE:
        ...
        {
            // Check for Freeze-Dry against Water types
            if (isFreezeDry && (gBattleMons[gBattlerTarget].type1 == TYPE_WATER || gBattleMons[gBattlerTarget].type2 == TYPE_WATER))
            {
                gMoveResultFlags &= ~MOVE_RESULT_NOT_VERY_EFFECTIVE; // Remove not very effective if it was set
                gMoveResultFlags |= MOVE_RESULT_SUPER_EFFECTIVE;
            }
    ...
    case TYPE_MUL_SUPER_EFFECTIVE:
        if (gBattleMoves[gCurrentMove].power && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
        {
            if (isFreezeDry && (gBattleMons[gBattlerTarget].type1 == TYPE_WATER || gBattleMons[gBattlerTarget].type2 == TYPE_WATER))
            {
                gMoveResultFlags |= MOVE_RESULT_SUPER_EFFECTIVE; // Ensure super effective is set for Freeze-Dry
            } 
...
static void Cmd_typecalc(void)
{
    s32 i = 0;
    u8 moveType;
    bool8 isFlyingPress = (gCurrentMove == MOVE_FLYING_PRESS);
    bool8 hasScrappy = (gBattleMons[gBattlerAttacker].ability == ABILITY_SCRAPPY);
    ...
    else if (gBattleMons[gBattlerAttacker].ability == ABILITY_LIQUID_VOICE && gBattleMoves[gCurrentMove].soundMove)
    {
        moveType = TYPE_WATER;
    }
    else if (gBattleMons[gBattlerAttacker].ability == ABILITY_REFRIGERATE && gBattleMoves[gCurrentMove].type == TYPE_NORMAL)
    {
        moveType = TYPE_ICE;
    }
    else if (gBattleMons[gBattlerAttacker].ability == ABILITY_PIXILATE && gBattleMoves[gCurrentMove].type == TYPE_NORMAL)
    {
        moveType = TYPE_FAIRY;
    }
    else if (gBattleMons[gBattlerAttacker].ability == ABILITY_AERILATE && gBattleMoves[gCurrentMove].type == TYPE_NORMAL)
    {
    ...
    } else if (IS_BATTLER_OF_TYPE(gBattlerAttacker, moveType) && gBattleMons[gBattlerAttacker].ability != ABILITY_ADAPTABILITY)
    {
        gBattleMoveDamage = (gBattleMoveDamage * 150) / 100;
    } 

    if (gStatuses3[gBattlerTarget] & STATUS3_MAGNET_RISE && moveType == TYPE_GROUND)
    {
        gMoveResultFlags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
        gLastLandedMoves[gBattlerTarget] = 0;
        gLastHitByType[gBattlerTarget] = 0;
        gBattleCommunication[MISS_TYPE] = B_MSG_ELECTROMAGNETISM_MISS;
    }
    else if (gBattleMons[gBattlerTarget].ability == ABILITY_LEVITATE && moveType == TYPE_GROUND && gBattleMons[gBattlerAttacker].ability != ABILITY_MOLD_BREAKER && gBattleMons[gBattlerAttacker].ability != ABILITY_TURBOBLAZE && gBattleMons[gBattlerAttacker].ability != ABILITY_TERAVOLT)
    {
    ...
    else
    {
        while (TYPE_EFFECT_ATK_TYPE(i) != TYPE_ENDTABLE)
        {
            ...
            else if (TYPE_EFFECT_ATK_TYPE(i) == moveType || (isFlyingPress && TYPE_EFFECT_ATK_TYPE(i) == TYPE_FLYING))
            {
                // check type1
    ...
    if ((gBattleMons[gBattlerTarget].ability == ABILITY_FILTER || gBattleMons[gBattlerTarget].ability == ABILITY_SOLID_ROCK) && (gMoveResultFlags & MOVE_RESULT_SUPER_EFFECTIVE) && gBattleMons[gBattlerAttacker].ability != ABILITY_MOLD_BREAKER && gBattleMons[gBattlerAttacker].ability != ABILITY_TURBOBLAZE && gBattleMons[gBattlerAttacker].ability != ABILITY_TERAVOLT)
    {
    ...
    if (gBattleMons[gBattlerTarget].ability == ABILITY_WONDER_GUARD && AttacksThisTurn(gBattlerAttacker, gCurrentMove) == 2
     ...
     && gBattleMons[gBattlerAttacker].ability != ABILITY_MOLD_BREAKER && gBattleMons[gBattlerAttacker].ability != ABILITY_TURBOBLAZE && gBattleMons[gBattlerAttacker].ability != ABILITY_TERAVOLT)
    {
...
static void CheckWonderGuardAndLevitate(void)
{
    ...
    if (gBattleMons[gBattlerTarget].ability == ABILITY_WONDER_GUARD && AttacksThisTurn(gBattlerAttacker, gCurrentMove) == 2 && gBattleMons[gBattlerAttacker].ability != ABILITY_MOLD_BREAKER && gBattleMons[gBattlerAttacker].ability != ABILITY_TURBOBLAZE && gBattleMons[gBattlerAttacker].ability != ABILITY_TERAVOLT)
    {
...
u8 TypeCalc(u16 move, u8 attacker, u8 defender)
{
    ...
    if (gStatuses3[defender] & STATUS3_MAGNET_RISE && moveType == TYPE_GROUND)
    {
        flags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
    }
    else if (gBattleMons[defender].ability == ABILITY_LEVITATE && moveType == TYPE_GROUND)
    {
        flags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
    }
    else
...
u8 AI_TypeCalc(u16 move, u16 targetSpecies, u8 targetAbility)
{
    ...
    else if (targetAbility == ABILITY_LEVITATE && moveType == TYPE_GROUND)
    {
        flags = MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE;
    }
    else
...
static void Cmd_adjustnormaldamage(void)
{
    ...
    // Implementing Sturdy ability effect
...
static void Cmd_critmessage(void)
{
    if (gBattleControllerExecFlags == 0)
    {
        ...
        || (gCritMultiplier == 3 && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)))
        {
...
static void Cmd_resultmessage(void)
{
    ...
    if (gMoveResultFlags & MOVE_RESULT_MISSED && (!(gMoveResultFlags & MOVE_RESULT_DOESNT_AFFECT_FOE) || (gBattleCommunication[MISS_TYPE] > B_MSG_AVOIDED_ATK && gBattleCommunication[MISS_TYPE] < B_MSG_ELECTROMAGNETISM_MISS)))
    {
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT && gBattleMons[gBattlerAttacker].ability != ABILITY_SHEER_FORCE)
    {
        ...
        case STATUS1_SLEEP:
            ...
            if (gBattleMons[gEffectBattler].ability == ABILITY_INSOMNIA)
                break;
            if (gBattleMons[gEffectBattler].ability == ABILITY_SWEET_VEIL)
                break;
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_WRAP:
                ...
                {
                    gBattleMons[gEffectBattler].status2 |= STATUS2_WRAPPED_TURN((Random() % 2) + 4); // 3-6 turns
            ...
            case MOVE_EFFECT_EVS_MINUS_1:
                if (gBattleMons[gBattlerAttacker].ability != ABILITY_SHEER_FORCE
                                && ChangeStatBuffs(SET_STAT_BUFF_VALUE(1) | STAT_BUFF_NEGATIVE,
            ...
            case MOVE_EFFECT_EVS_MINUS_2:
                if (gBattleMons[gBattlerAttacker].ability != ABILITY_SHEER_FORCE
                                && ChangeStatBuffs(SET_STAT_BUFF_VALUE(2) | STAT_BUFF_NEGATIVE,
            ...
            case MOVE_EFFECT_INCINERATE:
                ...
                break;
            case MOVE_EFFECT_V_CREATE: // V-Create
                BattleScriptPush(gBattlescriptCurrInstr + 1);
...
static void Cmd_playanimation(void)
{
    ...
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
    {
        ...
        gBattlescriptCurrInstr = BattleScript_Pausex20;
          ...
          || gBattlescriptCurrInstr[2] == B_ANIM_STRONG_WINDS)
    {
        BtlController_EmitBattleAnimation(BUFFER_A, gBattlescriptCurrInstr[2], *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 7;
...
static void Cmd_playanimation_var(void)
{
    ...
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
    {
        gBattlescriptCurrInstr += 10;
          ...
          || *animationIdPtr == B_ANIM_STRONG_WINDS)
    {
        BtlController_EmitBattleAnimation(BUFFER_A, *animationIdPtr, *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 10;
...
static void Cmd_playstatchangeanimation(void)
{
    ...
    u32 ability = gBattleMons[gActiveBattler].ability;
            ...
            if (statsToCheck & 1)
            {
                if (gBattlescriptCurrInstr[3] & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[gActiveBattler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && gBattleMons[gActiveBattler].ability != ABILITY_COMPETITIVE
...
static void Cmd_moveend(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].item == ITEM_ENIGMA_BERRY && gBattleMons[gBattlerAttacker].ability != ABILITY_KLUTZ)
        ...
        case MOVEEND_KINGS_SHIELD:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                if (gProtectStructs[gBattlerTarget].kingsShielded)
                {
                    ...
                    gBattleCommunication[MOVE_EFFECT_BYTE] = MOVE_EFFECT_ATK_MINUS_2;
                    BattleScriptPushCursor();
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_SPIKY_SHIELD:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                if (gProtectStructs[gBattlerTarget].spikyShielded && gBattleMons[gBattlerAttacker].ability != ABILITY_MAGIC_GUARD)
                {
                    gProtectStructs[gBattlerAttacker].touchedProtectLike = FALSE;
...
static void Cmd_typecalc2(void)
{
    ...
    else if (gBattleMons[gBattlerTarget].ability == ABILITY_LEVITATE && moveType == TYPE_GROUND)
    {
        gLastUsedAbility = gBattleMons[gBattlerTarget].ability;
        gMoveResultFlags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
        gLastLandedMoves[gBattlerTarget] = 0;
...
static void Cmd_adjustsetdamage(void)
{
    ...
    // Retrieve hold effect and parameter based on the item the target is holding
    if (gBattleMons[gBattlerTarget].item == ITEM_ENIGMA_BERRY && gBattleMons[gBattlerTarget].ability != ABILITY_KLUTZ)
    {
        holdEffect = gEnigmaBerries[gBattlerTarget].holdEffect;
        param = gEnigmaBerries[gBattlerTarget].holdEffectParam;
    ...
    // Check for Focus Band activation
    if (holdEffect == HOLD_EFFECT_FOCUS_BAND && (Random() % 100) < param)
    {
        RecordItemEffectBattle(gBattlerTarget, holdEffect);
        gSpecialStatuses[gBattlerTarget].focusBanded = 1;
    ...
    // Apply damage adjustment for False Swipe, Endure, Focus Band, and Sturdy
    if (!(gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE)
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_PSYCHO_SHIFT:
    {
        ...
        else if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE))
    ...
    {
        VARIOUS_ARGS(const u8 *failInstr);
        if (!(gSideStatuses[GetBattlerSide(gBattlerAttacker)] & SIDE_STATUS_LUCKY_CHANT))
        {
            gSideStatuses[GetBattlerSide(gBattlerAttacker)] |= SIDE_STATUS_LUCKY_CHANT;
            gSideTimers[GetBattlerSide(gBattlerAttacker)].luckyChantBattlerId = gBattlerAttacker;
            gSideTimers[GetBattlerSide(gBattlerAttacker)].luckyChantTimer = 5;
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        else
        {
    ...
    case VARIOUS_TRY_REFLECT_TYPE:
    {
        ...
        if (targetType1 == TYPE_MYSTERY && targetType2 == TYPE_MYSTERY)
        {
    ...
    case VARIOUS_JUMP_IF_MYSTIC_GUARD:
    {
        VARIOUS_ARGS(const u8 *jumpInstr);
        if (gBattleMons[gBattlerTarget].ability == ABILITY_MYSTIC_GUARD)
        {
            gBattlerAbility = gBattlerTarget;
...
static void Cmd_setprotectlike(void)
{
    ...
    if (lastMove != MOVE_PROTECT && lastMove != MOVE_DETECT && lastMove != MOVE_ENDURE && lastMove != MOVE_WIDE_GUARD && lastMove != MOVE_QUICK_GUARD && lastMove != MOVE_MAT_BLOCK && lastMove != MOVE_CRAFTY_SHIELD && lastMove != MOVE_KINGS_SHIELD)
...
static void Cmd_jumpifcantmakeasleep(void)
{
    ...
    if (UproarWakeUpCheck(gBattlerTarget))
    {
        gBattlescriptCurrInstr = jumpPtr;
            ...
            || gBattleMons[gBattlerTarget].ability == ABILITY_SWEET_VEIL)
    {
        gLastUsedAbility = gBattleMons[gBattlerTarget].ability;
...
static void Cmd_weatherdamage(void)
{
    ...
    if (WEATHER_HAS_EFFECT && gBattleMons[gBattlerAttacker].ability != ABILITY_MAGIC_GUARD)
    {
        if (gBattleWeather & B_WEATHER_SANDSTORM)
        {
        ...
        {
            if (gBattleMons[gBattlerAttacker].ability == ABILITY_ICE_BODY
...
static void Cmd_tryinfatuating(void)
{
    ...
    if (gBattleMons[gBattlerTarget].ability == ABILITY_OBLIVIOUS
     || gBattleMons[gBattlerTarget].ability == ABILITY_AROMA_VEIL)
    {
...
static bool8 IsTwoTurnsMove(u16 move)
{
    if (gBattleMoves[move].effect == EFFECT_SKULL_BASH
     ...
     || gBattleMoves[move].effect == EFFECT_KYUREM_PAR
     || gBattleMoves[move].effect == EFFECT_KYUREM_BRN
    ...
    else
    {
        ...
        else
        {
            ...
            *(u8 *)((u8 *)(&gBattleStruct->choicedMove[gBattlerAttacker]) + 1) = 0;
            ...
            {
                if (gBattleMons[gBattlerAttacker].ability == ABILITY_UNBURDEN && gBattleResources->flags->flags[gBattlerAttacker] & RESOURCE_FLAG_UNBURDEN)
...
static void Cmd_weightdamagecalculation(void)
{
    ...
    // Retrieve the weight of the target Pokémon
    actualWeight = GetPokedexHeightWeight(SpeciesToNationalPokedexNum(gBattleMons[gBattlerTarget].species), 1);
    
    // Check if the target Pokémon has the Heavy Metal ability, which doubles its weight
    if (gBattleMons[gBattlerTarget].ability == ABILITY_HEAVY_METAL)
        actualWeight *= 2;

    // Check if the target Pokémon has the Light Metal ability, which halves its weight
    if (gBattleMons[gBattlerTarget].ability == ABILITY_LIGHT_METAL)
        actualWeight /= 2;

    for (i = 0; sWeightToDamageTable[i] != 0xFFFF; i += 2)
    {
        if (sWeightToDamageTable[i] > actualWeight)
            break;
    }
...
static void Cmd_setroomterrain(void)
{
    ...
    case EFFECT_GRASSY_TERRAIN:
        HandleRoomMove(STATUS_FIELD_GRASSY_TERRAIN, &gFieldTimers.grassyTerrainTimer, 2);
        break;
    case EFFECT_MISTY_TERRAIN:
        HandleRoomMove(STATUS_FIELD_MISTY_TERRAIN, &gFieldTimers.mistyTerrainTimer, 4);
        break;
    case EFFECT_PSYCHIC_TERRAIN:
        HandleRoomMove(STATUS_FIELD_PSYCHIC_TERRAIN, &gFieldTimers.psychicTerrainTimer, 6);
        break;
    case EFFECT_ELECTRIC_TERRAIN:
        HandleRoomMove(STATUS_FIELD_ELECTRIC_TERRAIN, &gFieldTimers.electricTerrainTimer, 8);
        break;
    default:
...
static void Cmd_pickup(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if ((ability == ABILITY_PICKUP || ability == ABILITY_COLLECTOR) 
         ...
         && (Random() % 10) == 0)
        {
            ...
            {
                percentTotal += sPickupItems[j].percentage[lvlDivBy10];
                if (random < percentTotal)
                {
                    SetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM, &sPickupItems[j].itemId);
                    break;
                }
            }
        }
...
static void Cmd_trymagnetrise(void)
{
    ...
    if (gStatuses3[gBattlerAttacker] & STATUS3_MAGNET_RISE)
    {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
        gBattlescriptCurrInstr = cmd->failInstr;
    }
...
{
    u8 sideAttacker = GetBattlerSide(gBattlerAttacker);
    u8 sidePhysicalTarget = GetBattlerSide(gProtectStructs[gBattlerAttacker].physicalBattlerId);
    u8 sideSpecialTarget = GetBattlerSide(gProtectStructs[gBattlerAttacker].specialBattlerId);

    // Determine which damage is greater or combine them
    int totalPhysicalDamage = gProtectStructs[gBattlerAttacker].physicalDmg;
    int totalSpecialDamage = gProtectStructs[gBattlerAttacker].specialDmg;
    int totalDamage;

    // Selecting the maximum of physical or special damage
    if (totalPhysicalDamage > totalSpecialDamage)
        totalDamage = totalPhysicalDamage * 1.5;
    else
        totalDamage = totalSpecialDamage * 1.5;

    // Setting the target based on which damage type was greater
    if (totalPhysicalDamage > totalSpecialDamage && sideAttacker != sidePhysicalTarget && gBattleMons[gProtectStructs[gBattlerAttacker].physicalBattlerId].hp)
        gBattlerTarget = gProtectStructs[gBattlerAttacker].physicalBattlerId;
    else if (totalSpecialDamage >= totalPhysicalDamage && sideAttacker != sideSpecialTarget && gBattleMons[gProtectStructs[gBattlerAttacker].specialBattlerId].hp)
        gBattlerTarget = gProtectStructs[gBattlerAttacker].specialBattlerId;

    if (totalDamage > 0 && gBattleMons[gBattlerTarget].hp)
    {
        gBattleMoveDamage = totalDamage;
        gBattlescriptCurrInstr += 5;
    }
    else
    {
